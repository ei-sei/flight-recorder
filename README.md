...
- **Speech pace (WPM)** - live words-per-minute estimate using the browser's built-in speech recognition, plus the recogniser's average confidence for the recording (a real signal from the same recogniser, not a fabricated score). Off by default; see [Data & privacy](#data--privacy) below. Only available on Chromium-based webviews (Windows).
...
- **Metadata** - the question bank, and each attempt's question/category/date/duration/score/notes/response-delay - is persisted locally via `tauri-plugin-store`, a JSON file in the app's local data directory. When Speech pace (WPM) is on, the transcript and the recogniser's average confidence are saved alongside it.
