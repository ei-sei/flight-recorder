...
- **Metadata** - the question bank, and each attempt's question/category/date/duration/score/notes/response-delay - is persisted locally via `tauri-plugin-store`, a JSON file in the app's local data directory. When Speech pace (WPM) is on, the transcript is saved alongside it.
