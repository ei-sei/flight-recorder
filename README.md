...
A local desktop app for practising job interviews on webcam. Built with Tauri. Runs fully on your machine - no cloud storage of video or data, except for the optional, opt-in speech-pace (WPM) feature.
...
- **Webcam recorder** with a live viewfinder, record/stop tied to the selected question, and adjustable camera/microphone/quality (720p/1080p) in Settings. A date/timer watermark is burned into the saved recording itself (not just shown live), and a live voice waveform is shown while recording.
...
- **Video recordings** are written straight to disk under your OS "Videos" folder: `Videos/flight-recorder/{category}/{YYMMDD}-a{attempt number}-{question abbreviation}.webm` - e.g. `260901-a2-tmatydwt.webm` for the second attempt at "Tell me about a time you disagreed with a teammate." The extension follows whatever format the webview can actually record - `.webm` (VP8/VP9+Opus) normally, falling back to `.mp4` (H.264) on webviews that don't support recording WebM (notably Safari/WKWebView). A date/timer watermark (British DD/MM/YYYY format) is composited into the video itself before it's saved, camcorder-style. Nothing about video ever leaves the machine.
- **Metadata** - the question bank, and each attempt's question/category/date/duration/score/notes/response-delay - is persisted locally via `tauri-plugin-store`, in `library.json` inside that same `Videos/flight-recorder/` folder (not a hidden app-data directory). When Speech pace (WPM) is on, the transcript is saved alongside it.
