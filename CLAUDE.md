...
- Filler-word count: shown only when a transcript exists. Best-effort by nature — speech recognition strips fillers unpredictably (Google's engine drops "um"/"uh" almost entirely, Whisper keeps them inconsistently), so a zero means "none survived transcription", not "none said". The pause figures are the trustworthy signal; treat fillers as a bonus and don't build anything load-bearing on them.
