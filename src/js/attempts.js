...
//
// Split on both kinds of slash before joining: recordings made on Windows
// were stored as "behavioural\\file.mp4", which on Linux or macOS is one
// file name with a backslash in it rather than a folder and a file - so a
// library copied over from Windows found none of its videos. Every name in
// these paths is generated by the app, so neither slash can appear inside one.
export async function resolveVideoPath(relativePath) {
  return join(await libraryDir(), ...relativePath.split(/[\\/]/));
}
...
async function computeVideoRelativePath(question, date, extension) {
  ...
  // Always a forward slash, whatever this machine uses: it's stored in
  // library.json, which has to read the same on every OS.
